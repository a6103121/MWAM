...
#
#         with torch.no_grad():
#             outputs_batch,_,_,_ = model(img_rgb, img_depth)
#             # if isinstance(outputs_batch, tuple):
#             #     outputs_batch = outputs_batch[0]
#             # print(outputs_batch)
#             outputs_batch = nn.Softmax(1)(outputs_batch)
...
#         #                                                                       np.int32(args.train_epoch * 2 / 6),
#         #                                                                       np.int32(args.train_epoch * 3 / 6)])
#         cos_scheduler = optim.lr_scheduler.MultiStepLR(optimizer, milestones=[70,80,90])
#         if args.lr_warmup:
#             scheduler_warmup = GradualWarmupScheduler(args, optimizer, multiplier=1,
#                                                       after_scheduler=cos_scheduler)
#     else:
...
#
#             # model.args.epoch = epoch
#             output,weights,vis_res,depth_res= model(img_rgb, img_depth)
#
#             # if isinstance(output, tuple):
#             #     output = output[0]
#             # print(output)
#             loss = cost(output, target) + weights[0]*cost(vis_res, target) + weights[1]*cost(depth_res, target)
#
#             train_loss += loss.item()
#             loss.backward()
...
#                     pram.grad = pram.grad * weights[0]
#                     # print(name)
#                 if ("visual_net" in name):
#                     pram.grad = pram.grad * weights[1]
#                     # print(name)
#             optimizer.step()
#
#         # testing
...
def train_knowledge_distill_patch_feature_auxi_weak(net_dict, cost_dict, optimizer, train_loader, test_loader, args):
    ...
    while epoch < epoch_num:
        ...
        for batch_idx, batch_sample in enumerate(
            ...
            student_whole_out, student_layer3, student_layer4, x_rgb_out, x_ir_out, x_depth_out, p,weights = student_model(
            ...
            for name,pram in student_model.named_parameters():
                if ("special_bone_rgb" in name):
                    pram.grad = pram.grad * weights[0]
                if ("special_bone_depth" in name):
                    pram.grad = pram.grad * weights[1]
                if ("special_bone_ir" in name):
                    pram.grad = pram.grad * weights[2]
